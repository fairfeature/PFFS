...
def collectdata(datasetname,protectedattribute,datapath):
    ...
    #     featurenumlist = np.arange(1, len(dataset_orig.feature_names), 2)
    # else:
...
def drawFig(datasetname,protectedattribute,filepath, ax1):
    newpath = filepath.replace('.csv','_average.csv')
...
def runall():
    ...
    xy = [241,242,243,244,245,246,247,248]
    ...
    for i in range(len(datasetnamelist)):
        ...
        ax1 = plt.subplot(xy[i])
        filepath = './newresults/' + datasetnamelist[i][0] + '-' + datasetnamelist[i][1] + '-2d-featurenum.csv'
        #collectdata(i[0], i[1], filepath)
        #lib.get_average(filepath)
        drawFig(datasetnamelist[i][0], datasetnamelist[i][1], filepath, ax1)

    lines, labels = fig.axes[-1].get_legend_handles_labels()

    fig.legend(lines, labels,  ncol= 5, loc='upper center', fontsize = 24 , bbox_to_anchor=(0.5, 0.95))#

    #fig.legend(loc='upper center')
    plt.savefig('./plots/' + 'figure.pdf')#, bbox_inches='tight'
    ...
    import matplotlib.pyplot as plt
    plt.figure()
    ax[0] = plt.subplots(221)
    ax[1] = plt.subplots(222)
    plt.show()
    '''
