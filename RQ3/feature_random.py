...
def drawFig(datasetname, protectedattribute, filepath):
    ...
    ax1.set_title(datasetname + ' - ' + protectedattribute.lower() + ' - random', fontsize=25, fontweight='bold')
    ...
    if 'adult' or 'compas' in datasetname:
        plt.xticks(np.arange(min(range_), max(range_) + 1, int(len(range_)/3)))
...
def runall():
    ...
    for i in datasetnamelist:
        filepath = './results/' + i[0] + '-' + i[1] + '-random.csv'
