...
def collectdata(datasetname,protectedattribute,datapath):
    ...
    from filter import slopeTest, fisher3
    ...
    for turn in np.arange(0, 50, 1):
        ...
        featureset_index, featureset = fisher3(datasetname, dataset_orig_train, protectedattribute)

        np.savetxt(datasetname + '_' + protectedattribute + '/feature_list/run_' + str(turn) + '.csv', featureset_index)

        rvalue_list, pvalue_list, slope_list, intercept_list = slopeTest(protectedattribute, datasetname, dataset_orig_train, featureset)
        np.savetxt(datasetname + '_' + protectedattribute + '/rvalue_list/run_' + str(turn) + '.csv', rvalue_list)
        np.savetxt(datasetname + '_' + protectedattribute + '/pvalue_list/run_' + str(turn) + '.csv', pvalue_list)
        np.savetxt(datasetname + '_' + protectedattribute + '/slope_list/run_' + str(turn) + '.csv', slope_list)
        np.savetxt(datasetname + '_' + protectedattribute + '/intercept_list/run_' + str(turn) + '.csv', intercept_list)
...
def runall():
    datasetnamelist = [['adult', 'sex'], ['adult', 'race'], ['bank', 'age'], ['german', 'age'], ['compas', 'sex'], ['compas', 'race'], ['meps', 'RACE'], ['german', 'sex']]#[['adult', 'sex'], ['adult', 'race'], ['bank', 'age']]#
